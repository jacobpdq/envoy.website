...
<h2><?php echo __('Order Item');?></h2>
	<dl><?php $i = 0; $class = ' class="altrow"';?>
		...
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $orderItem['OrderItem']['qty_ordered']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>><?php echo __('Qty Shipped'); ?></dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $orderItem['OrderItem']['qty_shipped']; ?>
			&nbsp;
		</dd>
	</dl>
</div>
<div class="actions">
	...
	<ul>
		...
		<li><?php echo $this->Html->link(__('Delete Order Item'), array('action' => 'delete', $orderItem['OrderItem']['id']), null, sprintf(__('Are you sure you want to delete # %s?'), $orderItem['OrderItem']['id'])); ?> </li>
